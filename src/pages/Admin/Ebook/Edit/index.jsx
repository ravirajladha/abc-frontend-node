...
import { updateEbook, fetchEbookDetails } from '@/api/admin';
...
function Edit() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetchEbookDetails(ebookId);
      const ebookData = response.ebook;
      fetchSubjectsDropdownData(ebookData.subject_id);

      setFormData({
        subject: ebookData.subject_id,
        course: ebookData.course_id,
        title: ebookData.ebook_title,
        image: null,
        description: ebookData.ebook_description,
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateEbook(ebookId, submissionData);
  ...
  return (
    <div>
      <ContentHeader title="Edit eBook" />
      {loading ? (
        <ContentLoader />
      ) : (
        <ContentFormWrapper formTitle="Update eBook">
