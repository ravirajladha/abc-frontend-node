...
import { ContentHeader, ContentLoader, TextEditor } from '@/components/common';
import { fetchCourseData, fetchTrainersDropdown } from '@/api/common';
...
function Edit({ title }) {
  ...
  const [isFetching, setIsFetching] = useState(true); // To manage fetching state
  ...
  const handleSubmit = async (event) => {
    ...
    if (formData.course_image) submissionData.append('course_image', formData.course_image);
    ...
    try {
      const response = await updateCourse(courseId, submissionData);
  ...
  const fetchData = async () => {
    try {
      const trainersData = await fetchTrainersDropdown();
      setTrainers(trainersData);

      const courseData = await fetchCourseData(courseId);
      setFormData({
        course_name: courseData.name,
        course_image: null, // Image would be replaced only if user uploads a new one
        course_image_name: courseData.course_image_name || '',
        benefits: courseData.benefits || '',
        description: courseData.description || '',
        access_validity: courseData.access_validity || '',
        course_video: null, // Video would be replaced only if user uploads a new one
        course_video_name: courseData.course_video_name || '',
              ...
              <div className="row">
                <div className="col-lg-6 col-md-12 mb-3">
                  <div className="form-group">
                    <label className="mont-font fw-600 font-xsss">Course Name</label>
                    <input
                      type="text"
                    ...
                    {validationErrors.course_name && (
                      <span className="text-danger">{validationErrors.course_name}</span>
                    )}
                  </div>
                </div>
                <div className="col-lg-6 col-md-12 mb-3">
                  <div className="form-group">
                    <label className="mont-font form-label fw-600 font-xsss">Course Image</label>
                    <input
                      type="text"
                      className="form-control"
                    ...
                    {validationErrors.course_image && (
                      <span className="text-danger">{validationErrors.course_image}</span>
                    )}
                  </div>
                </div>
                <div className="col-lg-6 col-md-12 mb-3">
                  <div className="form-group">
                    <label className="mont-font fw-600 font-xsss">Course Validity</label>
                    <input
                      type="number"
                      className="form-control"
                    ...
                    {validationErrors.access_validity && (
                      <span className="text-danger">{validationErrors.access_validity}</span>
                    )}
                  </div>
                </div>
                <div className="col-lg-6 col-md-12 mb-3">
                  <div className="form-group">
                    <label className="mont-font form-label fw-600 font-xsss">Course Video</label>
                    <input
                      type="text"
                      className="form-control"
                      placeholder="Select Course Video"
                      value={formData.course_video_name}
                      onClick={() => document.getElementById('courseVideoInput').click()}
                      readOnly
                    />
                    <input
                      type="file"
                ...
                <div className="col-lg-6">
                  <div className="form-group">
                    <label className="mont-font fw-600 font-xsss">Trainer</label>
                    <select
                      className="form-control"
                    ...
                    {validationErrors.trainer_id && (
                      <span className="text-danger">{validationErrors.trainer_id}</span>
                    )}
                  </div>
                </div>
                <div className="col-lg-12 col-md-12 mb-3">
                  <div className="form-group">
                    ...
                    {validationErrors.benefits && (
                      <span className="text-danger">{validationErrors.benefits}</span>
                    )}
                  </div>
                </div>
